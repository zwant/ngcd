...
const millisecondsToMinutesSeconds = (ms) => {
  ...
  return Math.floor(duration.asSeconds()) >= 60 ? `${fromMinutes <= 9 ? `0${fromMinutes}` : fromMinutes}h ${fromSeconds <= 9 ? `0${fromSeconds}` : fromSeconds}`
  : `00m ${fromSeconds <= 9 ? `0${fromSeconds}` : fromSeconds}s`;
};

export class PipelineDetails extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  render() {
